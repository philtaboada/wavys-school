...
const SubjectForm = ({
  ...
  useEffect(() => {
    console.log("Profesores disponibles:", relatedData?.teachers?.length || 0);
    console.log("Profesores seleccionados:", watchedTeachers);
  }, [relatedData?.teachers, watchedTeachers]);
    ...
    if (type === "create") {
      createMutation.mutate({
        ...
        onError: (error: any) => {
          ...
          toast.error(`Error al crear la asignatura: ${error.message || "Ocurrió un error desconocido"}`);
          setCustomError(error.message || "Ocurrió un error al crear la asignatura");
        }
      });
    } else if (type === "update") {
      updateMutation.mutate({
        ...
        onError: (error: any) => {
          ...
          toast.error(`Error al actualizar la asignatura: ${error.message || "Ocurrió un error desconocido"}`);
          setCustomError(error.message || "Ocurrió un error al actualizar la asignatura");
        }
      });
    }
  });
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 p-4 sm:p-6 rounded-t-xl shadow-md relative overflow-hidden w-full">
        ...
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white relative z-10 flex items-center gap-2 sm:gap-3">
          <span className="bg-white/20 p-2 rounded-lg">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white hidden sm:block">
              <path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"></path>
            </svg>
          </span>
          {type === "create" ? "Crear nueva asignatura" : "Actualizar asignatura"}
        </h1>
        <p className="text-white/80 mt-2 max-w-2xl relative z-10">
          Complete la información a continuación para registrar una nueva asignatura en el sistema
        </p>
      </div>

      
      <div className="p-4 sm:p-6 bg-white rounded-b-xl shadow-md">
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="teachers" className="text-sm font-medium flex items-center gap-1">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            id="teachers"
            multiple
            className={`p-2 border rounded-md h-32 ${errors.teachers ? 'border-red-500' : 'border-gray-300'}`}
            {...register("teachers")}
            >
            ...
            {!relatedData?.teachers || relatedData.teachers.length === 0 ? (
              <p className="text-amber-600 text-xs mt-1">
                No se han cargado profesores. Por favor, cierre el formulario y vuelva a intentarlo.
              </p>
            ) : (
                <p className="text-xs text-gray-500 mt-1">
                  Mantenga presionada la tecla Ctrl (o Cmd en Mac) para seleccionar múltiples profesores
            </p>
            )}
            {errors.teachers && (
