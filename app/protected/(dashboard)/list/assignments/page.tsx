...
export default async function AssignmentListPage({
  ...
  const renderRow = (item: Assignment) => {
    console.log('DEBUG - Renderizando fila tarea:', item);
    return (
      <tr
        key={item.id}
  ...
  // Construir la consulta base para la tabla Assignment
  let query = supabase
    .from('Assignment')
    .select(`
      id,
  ...
  if (role === "teacher") {
    ...
    if (!lessonError && lessonIds && lessonIds.length > 0) {
      ...
      // Si no hay lecciones, no mostramos tareas
      return (
  ...
  console.log('DEBUG - Resultados:', { 
    dataLength: assignmentData?.length, 
    count, 
    error: error ? JSON.stringify(error) : null,
    primerRegistro: assignmentData && assignmentData.length > 0 ? JSON.stringify(assignmentData[0]) : null
  });
